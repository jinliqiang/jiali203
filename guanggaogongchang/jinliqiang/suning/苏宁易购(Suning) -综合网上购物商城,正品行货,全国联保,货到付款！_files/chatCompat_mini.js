...
var talkDomain = (function() {
	...
	var pre = hostname.match(/(pre)*[.](cnsuning)[.]/i);
	if (pre != null) {
		return 'http://talk8pre.cnsuning.com';
	}
	var sit = hostname.match(/(sit)*[.]cnsuning[.]/i);
	if (sit != null) {
...
function getWebcallParam(prdurl, pn, or, sc, prodNo, page) {
	var p = [];
	if (prdurl) {
...
function findpassNoModalBrand(h, f, i, j) {
	...
	if (h != "") {
		window.open(domain + "/index.htm?brandId=" + h + "&flag=" + f
				+ "&groupId=" + i + "&url=" + encodeURIComponent(j), "_blank");
	} else {
 ...
 * @param gid 商家通道
 */
function findpassNewSupplier(pno, page, sc, prdurl, or, gid) {
	var domain = talkDomain;
	...
	catalogId = snT.catalogId || '';
	storeId = snT.storeId || '';
	var url = domain + "/index.htm?channelId=" + gid;
	window.open(url + "&catalogId=" + catalogId + '&storeId='
			+ storeId + '&'
			+ getWebcallParam(prdurl, "", or, sc, pno, page), "_blank");
}
 ...
 *            四级投影地址(非必须)
 */
function findpassNewBrand(pno, page, groupMember, classCode, brandId, groupID, prdurl) {
	var snT = sn || {};
	var domain = talkDomain;
	var sc = snT.vendorCode || '';
	var shopCode = snT.flagshipId || '0000000000';
	window.open(domain + "/index.htm?groupMember=" + groupMember
			+ "&classCode=" + classCode + "&brandId=" + brandId + "&groupId="
			+ groupID + "&" + getWebcallParam(prdurl, '', '', '', pno, page)
			+ "&sc=" + sc + "&shopCode=" + shopCode + "&rt=1", "_blank");
}